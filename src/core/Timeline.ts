...
export class Timeline {
  ...
  setOnReachedEnd(cb: (() => void) | undefined) {
  ...
  render() {
    const f = this.frames[this.idx] ?? ({ nodes: [], edges: [], label: "" } as any);
  ...
  private animateTo(targetIdx: number, done?: () => void, respectPause: boolean = false) {
    ...
    const dur = Math.max(20, to.durationMs ?? this.speed);
